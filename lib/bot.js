...
try {
    ...
    if (!fs.existsSync(sessionFile)) {
        ...
        logger.info('Created empty session file:', sessionFile);
    }
} catch (error) {
    logger.error('Error initializing session middleware:', error);
}
...
function isGreeting(text) {
    const greetings = ['hi', 'hello', 'hey', 'hola', 'buenos dias', 'buenas'];
...
bot.onText(/\/start/, async (msg) => {
    ...
    const welcomeMessage = `Welcome to Hornerito Bot! 🤖\n\n` +
